...
# compile regexes
if args_parsed.ar:
    reg_AR = re.compile("(?<=ApproachRate:)(\d)+(\.)*\d*")
if args_parsed.cs:
    reg_CS = re.compile("(?<=CircleSize:)(\d)+(\.)*\d*")
if args_parsed.hp:
    reg_HP = re.compile("(?<=HPDrainRate:)(\d)+(\.)*\d*")
